...
abstract class WebSocketServer {
  ...
  protected function broadcast($message) {
     ...
     print "broadcasting to total sockets=[" . sizeof($this->sockets) . "] total users=[" . sizeof($this->users) . "]" . PHP_EOL; 
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          if ($client < 0) {
            ...
            print "total sockets=[" . sizeof($this->sockets) . "] total users=[" . sizeof($this->users) . "]" . PHP_EOL;
          }
        } 
        else {
